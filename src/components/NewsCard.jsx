...
export default function NewsCard({ tag, title, date, excerpt, link = '#' }) {
  return (
    <motion.div
      whileHover={{ y: -2, boxShadow: '0 12px 24px rgba(0,0,0,0.05)', borderColor: 'rgba(201,169,110,0.4)' }}
      className="bg-white/70 backdrop-blur-xl border border-white/50 rounded-2xl shadow-sm transition-all overflow-hidden"
    >
      <Link href={link} className="flex items-center gap-4 p-4 md:p-5 h-full">
        ...
        <div className="flex-1 min-w-0">
          ...
          <h3 className="font-serif font-semibold text-dark text-base md:text-lg truncate">
            {title}
          </h3>
          <div className="relative mt-1 max-h-[2.4rem] overflow-hidden">
            <p className="text-sm text-gray-500 leading-snug">{excerpt}</p>
            {/* Градиент "текст в туман" */}
            <div className="absolute bottom-0 left-0 right-0 h-6 bg-gradient-to-t from-white/70 to-transparent pointer-events-none" />
          </div>
        </div>
